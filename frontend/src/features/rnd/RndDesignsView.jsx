  ...
  return (
    <button type="button" onClick={onOpen} data-testid={`design-card-${d.id}`}
      ...
      <div className="relative flex h-28 items-center justify-center bg-[#F5F5F7]">
        {cover ? <img src={designFileUrl(d.id, cover.id)} alt={d.title} data-testid={`design-cover-${d.id}`} className={`h-full w-full object-cover ${d.on_hold ? "opacity-70" : ""}`} loading="lazy" />
